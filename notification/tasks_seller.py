...
from .models import Coupon, GlobalDiscount
...
def send_sms_to_seller_canceled(order_data):
    """
    ارسال پیامک کنسلی سفارش به مغازه‌دار
    """
    try:
        order = Order.objects.select_related('user').get(
            id=order_data['id'],
    ...
    for seller in sellers:
        print(f"Sending SMS to seller {seller.phone}:")
        print(message)
        print("=" * 60)
        # Uncomment when SMS service is ready:
        # send_sms(to=seller.phone, message=message)
        sent_count += 1

    return f"پیامک کنسلی سفارش {order.tracking_code} به {sent_count} فروشنده ارسال شد"


@shared_task
def send_sms_to_seller_daily_report():
    """
    تسک روزانه برای مغازه‌دار که هر روز ساعت ۸ صبح پیامک سفارش‌های امروز رو بفرسته
    """
    ...
    sent_count = 0
    for seller in sellers:
        print(f"Sending daily report to seller {seller.phone}:")
        print(full_message)
        print("=" * 60)
        # Uncomment when SMS service is ready:
    ...
    return f"گزارش روزانه به {sent_count} فروشنده ارسال شد"
