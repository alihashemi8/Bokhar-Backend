...
class WithdrawalRequest(models.Model):
    ...
    user = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="withdrawal_requests"
    )
    wallet = models.ForeignKey(
        "wallet.Wallet", on_delete=models.PROTECT, related_name="withdrawals"
    )
    amount = models.BigIntegerField(validators=[MinValueValidator(1)])

    # اجباری: مشخص می‌کند روی کدام تراکنش شارژ، استرداد انجام شود
    deposit_payment = models.ForeignKey(
        "wallet.PaymentSession",
        on_delete=models.PROTECT,
...
class RefundRequest(models.Model):
    ...
    user = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="refund_requests"
    )

    order = models.ForeignKey(
        "order.Order", on_delete=models.PROTECT, related_name="refund_requests"
    )
